...
# Counter availability: Yes - player has access to full barracks/range/stable counters
# Physical access: Yes - all objectives reachable by land, no water barriers
# Timing viability: Yes - starting army can defend initial base while building economy
...
# Add hero unit
hero = unit_manager.add_unit(PlayerId.ONE, unit_const=HeroInfo.GENGHIS_KHAN.ID, x=player_base_x+2, y=player_base_y+2)

# Starting army
for i in range(10):
    unit_manager.add_unit(PlayerId.ONE, unit_const=UnitInfo.SCOUT_CAVALRY.ID, x=player_base_x+i, y=player_base_y+10)
...
enemy_gate = unit_manager.add_unit(PlayerId.TWO, unit_const=BuildingInfo.GATE_NORTH_TO_SOUTH.ID, x=enemy_base_x-5, y=enemy_base_y)
...
for i in range(5):
    ...
    unit_manager.add_unit(PlayerId.GAIA, unit_const=OtherInfo.FORAGE_BUSH.ID, x=player_base_x+5+i, y=player_base_y+15)
    unit_manager.add_unit(PlayerId.GAIA, unit_const=UnitInfo.SHEEP.ID, x=player_base_x+i, y=player_base_y+20)
    unit_manager.add_unit(PlayerId.GAIA, unit_const=UnitInfo.DEER.ID, x=player_base_x+20+i, y=player_base_y+20)

# Allied camp
ally_x = quarter + 20
ally_y = quarter + 20
ally = unit_manager.add_unit(PlayerId.THREE, unit_const=HeroInfo.JOAN_OF_ARC.ID, x=ally_x, y=ally_y)
unit_manager.add_unit(PlayerId.THREE, unit_const=BuildingInfo.TOWN_CENTER.ID, x=ally_x+5, y=ally_y+5)
...
easy_trigger.new_effect.create_object(object_list_unit_id=UnitInfo.KNIGHT.ID, source_player=PlayerId.ONE, location_x=player_base_x, location_y=player_base_y)
...
initial.new_effect.display_instructions(display_time=10, message="<YELLOW>The Rise of the Mongols begins...")

ally_ai = trigger_manager.add_trigger("Ally AI Setup")
ally_ai.new_condition.timer(timer=1)
ally_ai.new_effect.patrol(object_list_unit_id=UnitInfo.KNIGHT.ID, source_player=PlayerId.THREE, location_x=ally_x+10, location_y=ally_y+10)
...
hero_start.new_effect.display_instructions(display_time=10, message="<BLUE>The tribes must be united...")
...
# Save scenario
scenario.write_to_file("rise_of_mongols.aoe2scenario")